...
class Event {
    ...
    public function registerUserForEvent($userId, $eventId) {
        $query = "INSERT INTO event_registrations (user_id, event_id, registration_date) VALUES (?, ?, NOW())";
        $stmt = $this->conn->prepare($query);
    ...
    public function createEvent($title, $description, $schedule, $location, $eventDate) {
        $query = "INSERT INTO events (title, description, schedule, location, event_date) 
                  VALUES (?, ?, ?, ?, ?)";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("sssss", $title, $description, $schedule, $location, $eventDate);
        return $stmt->execute();
    }

    public function updateEvent($id, $title, $description, $schedule, $location, $eventDate) {
        $query = "UPDATE events 
                  SET title = ?, description = ?, schedule = ?, 
                      location = ?, event_date = ? 
                  WHERE id = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("sssssi", $title, $description, $schedule, $location, $eventDate, $id);
        return $stmt->execute();
    }
